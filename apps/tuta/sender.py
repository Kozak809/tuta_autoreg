...
def send_tuta_email(username, password, to_address, subject, body, proxy_port=None, show_cursor=False, count=1, saved_config=None):
    with sync_playwright() as pw:
        # Если есть конфиг, используем его настройки для запуска
        if saved_config:
            browser_config = saved_config.get("browser_config", {})
            browser_config["headless"] = not show_cursor
            if proxy_port:
                browser_config["proxy"] = {"server": f"socks5://127.0.0.1:{proxy_port}"}
        ...
        try:
            ...
            page.wait_for_selector(new_mail_btn_selector, timeout=90000)
...
Пример использования:
  python mail_sender.py --to example@gmail.com --subject "Hello" --body "My message" --count 2 --accounts-num 5 --show
    ...
    parser.add_argument("--noshow", action="store_true", help="Disable GUI (default is enabled)")
    parser.add_argument("--noxvfb", action="store_true", help="Disable Xvfb (default is enabled)")
    
    args = parser.parse_args()
    
    # GUI/XVFB enabled by default
    show_gui = not args.noshow
    ...
    try:
        ...
        proxy_path = os.path.join(os.path.dirname(__file__), "data/proxy_list.txt")
        if not os.path.exists(proxy_path) or (os.path.exists(proxy_path) and os.stat(proxy_path).st_size == 0):
        ...
        total_sent = 0
        for acc in accounts_data[:min(len(accounts_data), args.accounts_num)]:
            username, password = acc["email"], acc["password"]
            
            # Загрузка конфига
            saved_config, _ = get_config_manager().resolve_path(acc.get("config_path"))
            
            pm = None
            port = None
            
            # 1. Пробуем родной прокси из конфига
            proxy_link = saved_config.get("proxy") if saved_config and "proxy" in saved_config else None
            
            # 2. Если нет в конфиге, берем случайный
            if not proxy_link and links:
                proxy_link = random.choice(links)
                print(f"[*] Родного прокси нет, выбран случайный: {proxy_link[:40]}...")
