...
def save_account_safe(email, password, recovery_code="N/A", config_path="N/A"):
...
def worker_task(worker_id, initial_count, show_cursor, headless):
    ...
    while True:
        ...
        port = 10000 + (worker_id * 1000) + (attempts % 900)
        ...
        try:
            ...
            if not success:
                # Если прокси "мертвый", не увеличиваем attempts (чтобы не менять порт слишком часто) 
                # и сразу переходим к следующему прокси
